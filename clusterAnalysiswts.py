    ...
    while repeat == True:
        ...
        for stock, ID  in membershipDict.items():
            ...
            for label, center in centroidDict.items():
                ...
                #print(wts[ID.Row]*100)
                zToCenDist = sum((wts[ID.Row]*100*Z[ID.Row,:] - centroid)**2)
        ...
        if membershipDict != updateDict:
            ...
            for cluster, row in membershipDict.values():
                objFn += sum((centroidDict[cluster][1] - wts[row]*Z[row,:])**2)
...
nClusters = 4
path = '/Users/KnownWilderness/Documents/Spring2018/TheoDataGit/'

#path = 'C:\\Users\\math\\downloads\\'
fileName = 'clusterData2.txt'
...
centroidDict = centroidComputer( membershipDict, Z)
#dataToPlot= [centroidDict[0][1],centroidDict[1][1],centroidDict[2][1],centroidDict[2][1] ]
#plot(dataToPlot, dates)

w = {}
alpha = .1
wts = [alpha*(1 - alpha)**(i) for i in range(nDays)]
print(sum(wts))
